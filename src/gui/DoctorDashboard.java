...
public class DoctorDashboard extends JFrame 
{
    ...
    public DoctorDashboard() 
    {
        ...
        profilePic.setIcon(new ImageIcon(new ImageIcon("images/profile_image.png").getImage().getScaledInstance(50, 50, Image.SCALE_SMOOTH)));
        ...
        queueTitle.setIcon(new ImageIcon(new ImageIcon("images/queue.png").getImage().getScaledInstance(55, 70, Image.SCALE_SMOOTH)));
        ...
        //prescripton button action listener to open the prescription writing interface which can be implemented as a new JFrame or a JDialog
        ...
        // Data base fatching logic for the patient details and vitals can be implemented here using JDBC and the DBconnection class
            try {
            ...
            if (rs.next()) {
                ...
                // PATIENT INFO
                patientInfo.setText("Patient ID: PAT-" +rs.getInt("patient_id") +"  •  " +rs.getInt("age") +" Years  •  " +rs.getString("gender"));
        ...
        //next patient button logic to update the patient record and load the next patient in the queue and also add consultant notes and the prescriptions automatically
    ...
    try {
        ...
        private void openPrescriptionDialog() 
        {
            ...
            //medicine list panel to hold the list of medicines fetched from the database and displayed in a scrollable pane
            ...
            //add madicine button to add the medicine from the text field to the combo box and also save it in the database with the current patient id and also add a delete button to remove the medicine from the combo box and also from the database
            ...
            //add madicine button action listener to add the medicine from the text field to the combo box and also save it in the database with the current patient id and also add a delete button to remove the medicine from the combo box and also from the database
            ...
            //create send button to send the prescription to the pharmacy and also save the prescription in the database with the current patient id and also reduce the stock of the medicine in the database automatically when the prescription is sent to the pharmacy
            ...
            //Auto search filter to search for medicines in the database and display them in the combo box as the user types in the medicine text field
            ...
            medicineField.addKeyListener(new KeyAdapter() 
            {
                ...
                public void keyReleased(KeyEvent e) {
                    ...
                    String search =
                            medicineField.getText().toLowerCase();
            ...
            //save multiple medicines with dosage and frequency to the database with the current patient id and also reduce the stock of the medicine in the database automatically when the prescription is sent to the pharmacy
            sendBtn.addActionListener(e -> {
                    ...
                    try {
                        ...
                        Component[] components =
                                medicineListPanel.getComponents();
                        ...
                        for(Component comp : components) {
                            ...
                            RoundedTextField dosage =
                                    (RoundedTextField) row.getComponent(1);

                            RoundedTextField frequency =
                                    (RoundedTextField) row.getComponent(2);

                            String medicine =
                                    medBox.getSelectedItem().toString();

                            medicine =
                                    medicine.split("\\(")[0].trim();
